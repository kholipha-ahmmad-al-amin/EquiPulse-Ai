...
export function DatePicker({
  ...
  invalid = false,
  required = false,
  minWidth = "w-full", // Defaulted to full width to fit container
  customIndicator,
  showTime = false,
  international = false,
  onChange,
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    ...
    if (event.key === "ArrowUp" || event.key === "ArrowDown") {
      ...
      const day = Math.max(1, Math.min(31, Number(base.day || 1) + delta));
...
export function HeroUIStyles() {
  return (
    <style>{`
      ...
      .date-picker{position:relative;display:inline-flex;flex-direction:column;gap:.25rem;color:rgb(var(--color-ink));font-family:Inter,ui-sans-serif,system-ui,sans-serif;overflow:visible}
      ...
      .date-picker.w-full{width:100%}
      .label{font-size:0.75rem;line-height:1rem;font-weight:700;color:rgb(var(--color-ink-soft))}
      .required-mark{color:#ef4444}
      .description{font-size:.875rem;line-height:1.25rem;color:rgb(var(--color-ink-soft))}
      
      .date-input-group{display:inline-flex;align-items:center;height:2.5rem;overflow:hidden;border-radius:.5rem;border:1px solid rgb(var(--color-line));background:rgb(var(--color-surface));box-shadow:0 1px 2px rgba(0,0,0,.04);outline:none;transition:border-color .16s ease,box-shadow .16s ease,background-color .16s ease}
      .date-input-group:hover{border-color:rgb(var(--color-accent));}
      .date-input-group:focus-within{border-color:rgb(var(--color-accent));box-shadow:0 0 0 2px rgba(var(--color-accent), .2)}
      ...
      .date-input-group[aria-disabled=true]{pointer-events:none;background:rgb(var(--color-muted));color:rgb(var(--color-ink-soft))}
      ...
      .date-input-group__segment[data-placeholder=true]{color:rgb(var(--color-ink-soft))}
      .date-input-group__segment:focus,.date-input-group__segment[data-focused=true]{background:rgba(var(--color-accent), .14);color:rgb(var(--color-accent))}
      .date-input-group__suffix{pointer-events:none;display:flex;align-items:center;flex-shrink:0;margin-right:.75rem;color:rgb(var(--color-ink-soft))}
      
      .date-picker__trigger{pointer-events:auto;display:inline-flex;align-items:center;justify-content:center;width:1.5rem;height:1.5rem;padding:.25rem;border:0;border-radius:.75rem;background:transparent;color:rgb(var(--color-ink-soft));cursor:pointer;transition:box-shadow .15s ease,background-color .16s ease,color .16s ease,transform .12s ease}
      .date-picker__trigger:hover{background:rgb(var(--color-accent));color:rgb(var(--color-surface))}
      .date-picker__trigger:active{transform:scale(.96)}
      .date-picker__trigger:focus-visible{outline:2px solid rgba(var(--color-accent), .5);outline-offset:2px}
      .date-picker__trigger-indicator{display:inline-flex;align-items:center;justify-content:center;font-size:1rem}
      
      .date-picker__popover{position:absolute;z-index:100;top:calc(100% + .5rem);left:0;width:var(--trigger-width,16rem);max-width:var(--trigger-width,16rem);overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;border-radius:min(32px,1.25rem);border:1px solid rgb(var(--color-line));background:rgb(var(--color-surface));box-shadow:0 16px 40px rgba(0,0,0,.16),0 2px 8px rgba(0,0,0,.08);padding:.75rem;transform-origin:top;animation:datePickerPopover .16s ease-out}
      ...
      .calendar-year-picker__trigger:hover,.calendar__nav-button:hover{background:rgb(var(--color-muted))}
      .calendar-year-picker__trigger-heading{font-size:.875rem}.calendar-year-picker__trigger-indicator{display:inline-flex;color:rgb(var(--color-ink-soft))}
      ...
      .calendar__header-cell{height:2rem;text-align:center;font-size:.75rem;font-weight:500;color:rgb(var(--color-ink-soft))}
      ...
      .calendar__cell:hover{background:rgb(var(--color-muted))}
      .calendar__cell:focus-visible{outline:2px solid rgb(var(--color-accent));outline-offset:1px}
      .calendar__cell:active{transform:scale(.95)}
      .calendar__cell[data-outside-month=true]{color:rgba(var(--color-ink-soft), .5)}
      .calendar__cell[data-selected=true]{background:rgb(var(--color-accent));color:rgb(var(--color-surface));font-weight:500}
      
      .calendar-year-picker__year-grid{max-height:9rem;overflow:auto;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.25rem;border-top:1px solid rgb(var(--color-line));padding-top:.5rem}
      ...
      .calendar-year-picker__year-cell[data-selected=true]{background:rgb(var(--color-accent));color:rgb(var(--color-surface))}
      ...
      .dark .date-picker__popover{box-shadow:0 18px 44px rgba(0,0,0,.55),0 2px 8px rgba(0,0,0,.32)}
    `}</style>
  );
}
